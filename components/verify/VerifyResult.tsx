...
export function VerifyResult({ data }: VerifyResultProps) {
  ...
  return (
    ...
    >
      ...
      <InfoRow
        label="Edice"
        value={`#${String(object.edition_number).padStart(3, '0')} z ${String(object.edition_total).padStart(3, '0')}`}
      />
      <InfoRow
        label="Aktuální majitel"
        value={`${currentOwnership.owner_name} (ověřeno)`}
      />
      <InfoRow
        label="Datum certifikátu"
        value={dateFormatter.format(new Date(currentOwnership.acquired_at))}
      />
      <InfoRow
        label="Převody"
        value={ownershipHistory.length <= 1 ? '0 (první majitel)' : `${ownershipHistory.length - 1}`}
      />
      <InfoRow
        label="Blockchain"
            ...
            {ownershipHistory.map((entry) => (
              ...
              >
                ...
                <span className={`text-[8px] tracking-[2px] uppercase ${entry.is_current ? 'text-success' : 'text-[rgba(245,242,236,0.3)]'}`}>
                  {entry.is_current ? 'Aktuální' : 'Předchozí'}
                </span>
              </div>
            ))}
          </div>
